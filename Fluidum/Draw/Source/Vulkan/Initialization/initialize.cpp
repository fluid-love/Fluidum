...
namespace FDR::Internal::Initialization {
	...
	void swapchain() {
		...
		UI32 queueFamilyIndices[] = { FVK::getGraphicsQueueFmily(BasePhysicalDeviceKey),FVK::getPresentQueueFmily(BasePhysicalDeviceKey) };
		...
		{
			const UI32 size = FVK::getSwapchainMinImageCount(BaseSwapchainKey);
	...
	void descriptorSetLayout() {
		...
		createInfo.bindingCount = static_cast<UI32>(bindings.size());
	...
	void graphicsPipeline() {
		...
		vertexInputInfo.vertexAttributeDescriptionCount = static_cast<UI32>(baseAttributeDescriptions.size());
	...
	void makeCommands() {
		...
		for (UI32 i = 0; i < FVK::getSwapchainMinImageCount(BaseSwapchainKey); i++) {
...
void FDR::Internal::Initialization::initialize(const FullScreenWithBarInfo& info) {
	...
	call(physicalDevice, "PhysicalDevice");
	call(logicalDevice, "LogicalDevice");
	call(swapchain, "Swapchain");
	call(renderpass, "RenderPass");
	call(descriptorSetLayout, "DescriptorsetLayout");
	call(shaderModule, "ShaderModule");
	call(graphicsPipelineLayout, "GraphicsPipelineLayout");
	call(graphicsPipeline, "GraphcsPipeline");
	call(colorImage, "ColorImage");
	call(colorImageView, "ColorImageView");
	call(depthImage, "Depthmage");
	call(commandPool, "CommandPool");
	call(commandBuffer, "CommandBuffer");
	call(frameBuffer, "FrameBuffer");
	call(sync, "Sync");
	call(queue, "Queue");
	call(descriptorPool, "DescriptorPool");
	call(setupImGui, "SetupImGui");
