...
template<>
class FS::LuaCheck::Check<FS::LuaType::Genome::SET> final : public FS::LuaCheck::Common<FS::LuaCheck::CommonType::GENOME> {
public:
	using Common::Common;
...
FS::LuaType::Ret FS::Lua::set_Genome(State L) {
	LuaCheck::Check<LuaType::Genome::SET> check(LuaName::Genome_Set, L, log, console);
	check.arg();
...

	{
		...
		((*genomes)[populationIndex])[pos] = true;
	}
